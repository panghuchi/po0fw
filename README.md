...
- **省坑位**：GET 先查，当前出口已在白名单则不 POST。白名单上限 5 个、写满按写入时间先进先出自动淘汰，无谓的写入会把常用 IP 挤出白名单。
- **蜂窝优化**：蜂窝（CGNAT）IP 频繁变化，自动写入设 10 分钟冷却，防止连续写入把常用 IP 挤出 FIFO 队列；面板手动刷新不受限。被挤掉的设备由其自身 cron/事件在几分钟内自愈补回。
- **即时响应**：network-changed 事件即时触发 + 每 3 分钟 cron 兜底。
...
| Egern | 同上（导入 Surge 模块） | 模块参数 `tokens` |
